...
export default async function middleware(request: NextRequest) {
  ...
  const accessToken: string | undefined =
    cookiesIntialise.get("accessToken")?.value;
  ...
  if (!accessToken) {
    ...
    try {
      const fetchVerify = await fetch(
        `${process.env.BACKEND_URL}/user/individual`,
        {
          headers: {
      ...
      try {
        const fetchNewTokens: any = await fetch(
          `${process.env.BACKEND_URL}/tokens`,
          {
            headers: {
...
export const config = {
  matcher: ["/", "/admin/:path*", "/scheduled-articles/:path*","/settings",],
};
